"""Spec-derived acceptance tests (feature 0008).
...
ACCEPTANCE = "specs/features/0008-corpus-and-metrics/acceptance.yaml"

#: Check vocabulary already in use across features 0001, 0002, 0004, 0005 and 0006. A criterion
...
@pytest.mark.spec
def test_0008_acceptance_file_is_wellformed(repo_root: Path):
    data = yaml.safe_load((repo_root / ACCEPTANCE).read_text())
    assert data["feature_id"] == "0008"
...
@pytest.mark.spec
def test_0008_every_criterion_names_an_existing_file(repo_root: Path):
    missing = []
...
@pytest.mark.spec
def test_0008_every_criterion_is_collectable(repo_root: Path):
